...
if [[ ! -d  $AQUA_DIAGNOSTICS ]] ; then
    ...
    source "$AQUA_DIAGNOSTICS/cli/util/logger.sh"
    log_message INFO "AQUA-diagnostics found in $AQUA_DIAGNOSTICS"
    log_message INFO "Sourcing logger.sh from: $AQUA_DIAGNOSTICS/cli/util/logger.sh"
fi
...
install_aqua() {
  ...
  cp ../../environment.yml $SCRIPTDIR/environment_hpc2020.yml
