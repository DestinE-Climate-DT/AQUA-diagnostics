...
def test_enso(tmp_path):
    ...
    enso.save_netcdf(enso.index, diagnostic="enso", diagnostic_product="index", outputdir=tmp_path)
    ...
    # Plotting
    plot_ref = PlotENSO(loglevel=loglevel, indexes=enso.index, ref_indexes=enso.index, outputdir=tmp_path)
    ...
    assert (
        os.path.exists(os.path.join(tmp_path, "pdf", "enso.regression_annual.ci.ERA5.era5-hpz3.r1.ci.ERA5.era5-hpz3.pdf"))
    ) is True
