...
def sample_lat_lon_data():
    ...
    def _make_data(mean_type="zonal", num_datasets=1, seasonal=False):
        ...
        def make_single_array(i=0):
            ...
            return xr.DataArray(
                np.random.rand(len(coord_values)) + i * 0.1, dims=[coord_name], coords={coord_name: coord_values}, attrs=attrs
            )
...
class TestPlotLatLonProfilesCore:
    ...
    def test_reference_data(self, sample_lat_lon_data):
        ...
        plotter = PlotLatLonProfiles(data=data, ref_data=ref_data, data_type="longterm", loglevel=loglevel)
    ...
    def test_custom_diagnostic_name_full(self, sample_lat_lon_data, tmp_path):
        ...
        plotter = PlotLatLonProfiles(data=data, data_type="longterm", diagnostic_name=custom_name, loglevel=loglevel)
    ...
    @pytest.mark.parametrize(
        "data_type,diagnostic_name,mean_type,expected_diagnostic,expected_product",
        [
    ...
    def test_diagnostic_product_construction(
        self, sample_lat_lon_data, tmp_path, data_type, diagnostic_name, mean_type, expected_diagnostic, expected_product
    ):
        ...
        plotter = PlotLatLonProfiles(data=data, data_type=data_type, diagnostic_name=diagnostic_name, loglevel=loglevel)
...
class TestPlotLatLonProfilesIntegration:
    ...
    def test_full_run(self, sample_lat_lon_data, tmp_path, data_type, format):
        ...
        plotter = PlotLatLonProfiles(data=data, data_type=data_type, loglevel=loglevel)

        plotter.run(outputdir=str(tmp_path), rebuild=True, format=format, dpi=DPI)
