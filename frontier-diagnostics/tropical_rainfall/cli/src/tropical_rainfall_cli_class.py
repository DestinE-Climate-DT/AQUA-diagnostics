...
class TropicalRainfallCLI:
    def __init__(self, config, args):
        ...
        self.reader = Reader(
            ...
            exp=self.exp,
            source=self.source,
            loglevel=self.reader_loglevel,
            regrid=self.regrid,
            nproc=self.nproc,
            **self.reader_kwargs,
        )
        self.diag = TropicalRainfall(
    ...
    def plot_histograms(self):
        ...
        filename = self.diag.dataset_to_netcdf_filename(
            start_year=self.s_year,
            end_year=self.f_year,
            start_month=self.s_month,
            end_month=self.f_month,
        ...
        for name, source in sources.items():
            ...
            filename = self.diag.dataset_to_netcdf_filename(
                path_to_netcdf=hist_path, name_of_file=f"histogram_{name}_{self.regrid}_{self.freq}"
            )
            if os.path.exists(filename) and not self.rebuild_output:
                self.logger.debug("File %s already exists, loading ...", filename)
                ...
                if merged_data is not None:
                    merged_data_sources[name] = merged_data
                    self.diag.dataset_to_netcdf(
                        merged_data, path_to_netcdf=hist_path, name_of_file=f"histogram_{name}_{self.regrid}_{self.freq}"
                    )
    ...
    def process_histograms(
        self, pdf_flag, pdf_p_flag, model_merged=None, mswep_merged=None, imerg_merged=None, era5_merged=None, linestyle="-"
    ):
        """
    ...
    def daily_variability(self):
        ...
        if "h" in self.freq.lower():
                    ...
                    for x in range(s_month, f_month + 1):
                        ...
                        if self.rebuild_output and self.diag.tools.find_files_with_keys(folder_path=path_to_output, keys=keys):
                            ...
                            try:
                                ...
                                self.diag.add_localtime(
                                    data,
                                    path_to_netcdf=path_to_output,
                                    name_of_file=f"{self.regrid}_{self.freq}",
                                    new_unit="mm/hr",
                                )
                            except KeyError:
            ...
            self.logger.warning(
                "Data appears to be not in hourly intervals. The CLI will not provide the plot of daily variability."
            )
    ...
    def plot_daily_variability(self):
        if "h" in self.freq.lower():
            ...
            filename = self.diag.dataset_to_netcdf(
                model_merged,
                path_to_netcdf=output_path,
                name_of_file=f"daily_variability_{self.model}_{self.exp}_{self.regrid}_{self.freq}",
            )

            add, _path_to_pdf = self.diag.daily_variability_plot(
                path_to_netcdf=filename,
                legend=legend,
                new_unit=self.new_unit,
                trop_lat=90,
                relative=False,
            ...
            if not os.path.exists(path_to_era5):
                ...
                return
            filename_era5 = self.diag.dataset_to_netcdf(
                era5_merged, path_to_netcdf=output_path, name_of_file="daily_variability_era5"
            )
            self.diag.daily_variability_plot(
                path_to_netcdf=filename_era5,
                legend="ERA5",
                relative=False,
                new_unit=self.new_unit,
                color=self.era5_color,
                add=add,
                linestyle="-",
                path_to_pdf=self.path_to_pdf,
                pdf_format=self.pdf_format,
    ...
    def average_profiles(self):
        ...
        if "m" in self.freq.lower() or "y" in self.freq.lower():
            ...
            model_average_path_lat = self.diag.average_into_netcdf(
            ...
            add, description = self._plot_and_add_metadata(
                model_average_path=model_average_path_lat, plot_title=plot_title, legend_model=legend_model, coord="lat"
            )
            add, description = self._plot_comparisons(
                add=add,
                model_average_path=model_average_path_lat,
                dataset_name="MSWEP",
                dataset_color=self.mswep_color,
                coord="lat",
                description=description,
            )
            add, description = self._plot_comparisons(
                ...
                model_average_path=model_average_path_lat,
                dataset_name="ERA5",
                dataset_color=self.era5_color,
                coord="lat",
                description=description,
            )

            model_average_path_lon = self.diag.average_into_netcdf(
                dataset=full_dataset,
                coord="lon",
                trop_lat=90,
                path_to_netcdf=output_path,
            ...
            add, description = self._plot_and_add_metadata(
                model_average_path=model_average_path_lon, plot_title=plot_title, legend_model=legend_model, coord="lon"
            )
            add, description = self._plot_comparisons(
                add=add,
                model_average_path=model_average_path_lon,
                dataset_name="MSWEP",
                dataset_color=self.mswep_color,
                coord="lon",
                description=description,
    ...
    def _plot_and_add_metadata(self, model_average_path, plot_title, legend_model, coord):
        ...
        add = self.diag.plot_of_average(
            path_to_netcdf=model_average_path,
            trop_lat=90,
            path_to_pdf=self.path_to_pdf,
            color=self.color,
            figsize=self.figsize,
    ...
    def _plot_comparisons(self, add, model_average_path, dataset_name, dataset_color, coord, description):
        ...
        add = self.diag.plot_of_average(
            path_to_netcdf=path_to_dataset,
            trop_lat=90,
            color=dataset_color,
            fig=add,
            legend=dataset_name,
