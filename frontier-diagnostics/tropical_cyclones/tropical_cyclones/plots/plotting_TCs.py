...
def multi_plot(tracks_nc_file, tdict, title=None, units=None, save=False):
    ...
    fig, axs = plt.subplots(
        nrows=3, ncols=3, figsize=(14, 14), subplot_kw={"projection": ccrs.PlateCarree()}, constrained_layout=True
    )
    axs = axs.flatten()
...
def plot_trajectories(trajfile, tdict):
    ...
    for i in range(nstorms):
        plt.scatter(
            x=xlon[i], y=xlat[i], color="black", s=15, linewidths=0.5, marker=".", alpha=0.8, transform=ccrs.PlateCarree()
        )  ## Important
    os.makedirs(tdict["paths"]["plotdir"], exist_ok=True)
    plt.savefig(
        tdict["paths"]["plotdir"] + f"tracks_{tdict['dataset']['model']}_{tdict['dataset']['exp']}.pdf",
        bbox_inches="tight",
        dpi=350,
    )
    plt.show()
