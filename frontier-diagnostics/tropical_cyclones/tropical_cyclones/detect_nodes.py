...
class DetectNodes():
    ...
    def run_detect_nodes(self, timestep):
        ...
        if 'z' in self.lowres2d.data_vars or self.orography:
            ...
            detect_string = f'DetectNodes --in_data {tempest_filein} --timefilter 6hr --out_header true --out {tempest_fileout} --out_file_format csv --searchbymin {tempest_dictionary["psl"]} ' \
