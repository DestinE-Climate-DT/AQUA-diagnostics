...
class DetectNodes():
    ...
    def run_detect_nodes(self, timestep):
        ...
        self.logger.warning(f'Tempest output file created:  {tempest_fileout}')
        ...
        if 'z' in self.lowres2d.data_vars or self.orography:
            self.logger.debug(f'Running DetectNodes with orography')
                        #detect_string = f'DetectNodes --in_data {tempest_filein} --timefilter 6hr --out_header --out {tempest_fileout} --out_file_format csv --searchbymin {tempest_dictionary["psl"]} ' \
            detect_string = f'DetectNodes --in_data {tempest_filein} --timefilter 6hr --out_header --out {tempest_fileout}  --searchbymin {tempest_dictionary["psl"]} ' \
