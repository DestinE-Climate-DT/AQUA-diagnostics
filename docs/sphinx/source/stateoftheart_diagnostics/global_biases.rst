...
* **StatGlobalBiases**: computes global statistics on the biases, such as mean bias and root mean square error (RMSE).
It can compute area-weighted statistics if grid cell areas are provided.
