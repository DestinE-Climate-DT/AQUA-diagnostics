...
def parse_arguments(args):
    ...
    parser = argparse.ArgumentParser(
        description=("Runs EnsembleZonal diagnostic, "),
...
def main(argv=None):
    ...
    for variable in cli.config_dict["diagnostics"]["ensemble"].get("variable", None):
        for region in cli.config_dict["diagnostics"]["ensemble"].get("region", None):
            ...
            cli.config_dict["diagnostics"]["ensemble"]["plot_params"]["default"].get("title_mean", None)
            cli.config_dict["diagnostics"]["ensemble"]["plot_params"]["default"].get("title_std", None)
            cli.config_dict["diagnostics"]["ensemble"]["plot_params"]["default"].get("cbar_label", None)
            cli.config_dict["diagnostics"]["ensemble"]["plot_params"]["default"].get("figure_size", None)
            ...
            if models is not None:
                ...
                if models[0]["realization"] is None:
                    models[0]["realization"] = extract_realizations_list(
                        catalog=models[0]["catalog"],
                ...
                for model in models:
                    ...
                    if model["realization"] is None:
                        model["realization"] = extract_realizations_list(
            ...
            if not description_mean:
                description_mean = "Ensemble mean zonal diagnostic"
            if not description_std:
                description_std = "Ensemble STD zonal diagnostic"
            ens_zm_plot.plot(
                **plot_arguments, dataset=ens_zm.dataset_mean, description=description_mean, data_name="ensemble_zonal_mean"
            )
            ens_zm_plot.plot(
                **plot_arguments, dataset=ens_zm.dataset_std, description=description_std, data_name="ensemble_zonal_std"
            )
