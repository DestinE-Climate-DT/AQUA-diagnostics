...
def main(argv=None):
    ...
    cli.logger.debug(f"Ensemble single model catalog: {catalog}, model: {model}, exp: {exp}, and source {source}")
    ...
    if "timeseries" in cli.config_dict["diagnostics"]["timeseries"]["run"]:
    ...
    if "globalbiases" in cli.config_dict["diagnostics"]["globalbiases"]["run"]:
