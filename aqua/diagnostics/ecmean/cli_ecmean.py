...
def parse_arguments(arguments):
    ...
    parser.add_argument("--nprocs", type=int, help="number of multiprocessing processes to use", default=1)
    parser.add_argument("-i", "--interface", type=str, help="non-standard interface file")
    parser.add_argument(
        "--source_oce",
        type=str,
...
def reader_data(
    ...
    try:
        reader = Reader(model=model, exp=exp, source=source, catalog=catalog, regrid=regrid, **reader_kwargs)
...
def set_title(diagnostic: str, model: str, exp: str, year1: int | None, year2: int | None) -> str:
    """
    ...
    builder = TitleBuilder(diagnostic=diag_name, model=model, exp=exp, startyear=year1, endyear=year2)
...
if __name__ == "__main__":
    ...
    logger.info("Running AQUA v%s Performance Indices diagnostic with ECmean4 v%s", aquaversion, eceversion)
    ...
    for dataset in config_dict["datasets"]:
        ...
        for diagnostic in ["global_mean", "performance_indices"]:
            ...
            metadata = outputsaver.create_metadata(diagnostic_product=diagnostic, metadata={"Description": description})
            ...
            if save_format:
                ...
                outputsaver.save_figure(
                    fig=ecmean_fig, diagnostic_product=diagnostic, metadata=metadata, rebuild=rebuild, extension=save_format
                )
