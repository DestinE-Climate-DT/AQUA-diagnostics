...
class PlotNAO(PlotBaseMixin):
    def __init__(self, indexes=None, ref_indexes=None, outputdir: str = "./", rebuild: bool = True, loglevel: str = "WARNING"):
        ...
        """
        super().__init__(
            indexes=indexes, ref_indexes=ref_indexes, diagnostic="nao", outputdir=outputdir, rebuild=rebuild, loglevel=loglevel
        )
    ...
    def plot_index(self, thresh: float = 0.0):
        ...
        fig, axs = indexes_plot(
            indexes=indexes, thresh=thresh, suptitle=title, ylabel="NAO index", labels=labels, loglevel=self.loglevel
        )
