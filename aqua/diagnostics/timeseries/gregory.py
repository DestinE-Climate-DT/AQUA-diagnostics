...
from aqua.core.util import DEFAULT_REALIZATION, convert_data_units
...
class Gregory(Diagnostic):
    ...
    def run(
        ...
        std: bool = False,
        t2m_name: str = "2t",
        net_toa_name: str = "tnlwrf+tnswrf",
        t2m_units: str = "degC",
        ...
        """
        self.retrieve(t2m=t2m, net_toa=net_toa, t2m_name=t2m_name, net_toa_name=net_toa_name, reader_kwargs=reader_kwargs)

        self.logger.info(f"Computing the Gregory Plot for the {freq} frequency.")
        if t2m:
            self.compute_t2m(freq=freq, std=std, units=t2m_units, var=t2m_name, exclude_incomplete=exclude_incomplete)
        if net_toa:
            # TODO: If needed add the units conversion for net_toa
            self.compute_net_toa(freq=freq, std=std, exclude_incomplete=exclude_incomplete)

        self.save_netcdf(freq=freq, std=std, t2m=t2m, net_toa=net_toa, outputdir=outputdir, rebuild=rebuild)
    ...
    def retrieve(
        ...
        if net_toa:
            self.net_toa = EvaluateFormula(
                data=data, formula=net_toa_name, short_name="net_toa", loglevel=self.loglevel
            ).evaluate()
    ...
    def compute_net_toa(self, freq: list = ["monthly", "annual"], std: bool = False, exclude_incomplete=True):
        """
        ...
        if t2m:
            if std and self.t2m_std is not None:
                super().save_netcdf(
                    ...
                    outputdir=outputdir,
                    rebuild=rebuild,
                    extra_keys={"var": "2t", "freq": "annual", "std": "std"},
                )
            if "monthly" in freq and self.t2m_monthly is not None:
                super().save_netcdf(
                    data=self.t2m_monthly,
                    diagnostic=self.diagnostic_name,
                    diagnostic_product=diagnostic_product,
                    outputdir=outputdir,
                    rebuild=rebuild,
                    extra_keys={"var": "2t", "freq": "monthly"},
                )
            if "annual" in freq and self.t2m_annual is not None:
                super().save_netcdf(
                    data=self.t2m_annual,
                    diagnostic=self.diagnostic_name,
                    diagnostic_product=diagnostic_product,
                    outputdir=outputdir,
                    rebuild=rebuild,
                    extra_keys={"var": "2t", "freq": "annual"},
                )
        if net_toa:
            if std and self.net_toa_std is not None:
                super().save_netcdf(
                    data=self.net_toa_std,
                    diagnostic=self.diagnostic_name,
                    diagnostic_product=diagnostic_product,
                    outputdir=outputdir,
                    rebuild=rebuild,
                    extra_keys={"var": "net_toa", "freq": "annual", "std": "std"},
                )
            if "monthly" in freq and self.net_toa_monthly is not None:
                super().save_netcdf(
                    data=self.net_toa_monthly,
                    diagnostic=self.diagnostic_name,
                    diagnostic_product=diagnostic_product,
                    outputdir=outputdir,
                    rebuild=rebuild,
                    extra_keys={"var": "net_toa", "freq": "monthly"},
                )
            if "annual" in freq and self.net_toa_annual is not None:
                super().save_netcdf(
                    data=self.net_toa_annual,
                    diagnostic=self.diagnostic_name,
                    diagnostic_product=diagnostic_product,
                    outputdir=outputdir,
                    rebuild=rebuild,
                    extra_keys={"var": "net_toa", "freq": "annual"},
                )
